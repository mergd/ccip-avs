...
import (
	...
	"github.com/mergd/ccip-avs/common"
	"github.com/mergd/ccip-avs/core/config"

	"github.com/mergd/ccip-avs/challenger/types"
...
type Challenger struct {
	...
	tasks              map[uint32]cstaskmanager.IIncredibleLendingTaskManagerTask
	taskResponses      map[uint32]types.TaskResponseData
	taskResponseChan   chan *cstaskmanager.ContractIncredibleLendingTaskManagerTaskResponded
...
func NewChallenger(c *config.Config) (*Challenger, error) {
	...
	challenger := &Challenger{
		...
		tasks:              make(map[uint32]cstaskmanager.IIncredibleLendingTaskManagerTask),
		taskResponses:      make(map[uint32]types.TaskResponseData),
		taskResponseChan:   make(chan *cstaskmanager.ContractIncredibleLendingTaskManagerTaskResponded),
...
func (c *Challenger) processNewTaskCreatedLog(newTaskCreatedLog *cstaskmanager.ContractIncredibleLendingTaskManagerNewTaskCreated) uint32 {
	c.tasks[newTaskCreatedLog.TaskIndex] = newTaskCreatedLog.Task
	return newTaskCreatedLog.TaskIndex
}

func (c *Challenger) processTaskResponseLog(taskResponseLog *cstaskmanager.ContractIncredibleLendingTaskManagerTaskResponded) uint32 {
...
func (c *Challenger) getNonSigningOperatorPubKeys(vLog *cstaskmanager.ContractIncredibleLendingTaskManagerTaskResponded) []cstaskmanager.BN254G1Point {
	...
	cstmAbi, err := abi.JSON(bytes.NewReader(common.IncredibleLendingTaskManagerAbi))
	if err != nil {
