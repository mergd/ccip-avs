...
function create_onchainoracle_binding {
    contract_dir=$1
    contract=$2
    binding_dir=$3
    echo "generating bindings for" $contract
    mkdir -p $binding_dir/${contract}
    contract_json="$contract_dir/out/src/${contract}.sol/${contract}.json"
    solc_abi=$(cat ${contract_json} | jq -r '.abi')
    solc_bin=$(cat ${contract_json} | jq -r '.bytecode.object')
...
avs_service_contracts="IncredibleLendingServiceManager IncredibleLendingTaskManager"
for contract in $avs_service_contracts; do
    create_binding . $contract ./bindings
done

create_binding . ERC20Mock ./bindings
create_onchainoracle_binding . OnChainDepthOracle ./bindings
